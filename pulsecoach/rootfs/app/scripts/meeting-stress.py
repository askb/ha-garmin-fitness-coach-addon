...
# Allow "import gcal" both when run as a script (sys.path[0] is this dir) and
# when imported via importlib by the tests. Append (not insert) with a dedup
# guard so the scripts dir never shadows stdlib/site-packages or piles up.
...
# Kept as thin wrappers so callers here read the same as before; gcal.py is
# shared with garmin-auth-server.py (link/unlink/list-calendars endpoints).
# --------------------------------------------------------------------------- #
